...
namespace rdb
{
    MemoryCache::FlushHandle:: FlushHandle(std::size_t index, bool ready)
        : idx(index), unlocked(ready) {}
    MemoryCache::FlushHandle::FlushHandle(FlushHandle&& copy) :
    ...
    bool MemoryCache::FlushHandle::ready() const noexcept
    {
        return unlocked.load(std::memory_order::acquire);
    }

    MemoryCache::Slot::Slot(DataType vtype, std::span<const unsigned char> data) noexcept
        : capacity(data.size()), size(data.size()), vtype(vtype)
    {
        std::memcpy(buffer().data(), data.data(), data.size());
    }
    MemoryCache::Slot::Slot(DataType vtype, std::size_t size) noexcept
        : capacity(size), size(size), vtype(vtype) {}

    std::span<const unsigned char> MemoryCache::Slot::buffer() const noexcept
    {
        return std::span(
    ...
    std::span<unsigned char> MemoryCache::Slot::buffer() noexcept
    {
        return std::span(
    ...
    std::span<const unsigned char> MemoryCache::Slot::flush_buffer() const noexcept
    {
        return std::span(
    ...
    std::span<unsigned char> MemoryCache::Slot::flush_buffer() noexcept
    {
        return std::span(
    ...
    std::size_t MemoryCache::Slot::allocation_size(DataType vtype, std::span<const unsigned char> data) noexcept
    {
        return sizeof(std::uint32_t) * 2 + sizeof(DataType) + data.size();
    }
    std::size_t MemoryCache::Slot::allocation_size(DataType vtype, std::size_t size) noexcept
    {
        return sizeof(std::uint32_t) * 2 + sizeof(DataType) + size;
    }

    MemoryCache::Slot* MemoryCache::SlotDeleter::allocate(DataType vtype, std::span<const unsigned char> data) noexcept
    {
        return ct::ordered_byte_map<Slot>::allocate_node(vtype, data);
    }
    MemoryCache::Slot* MemoryCache::SlotDeleter::allocate(DataType vtype, std::size_t size) noexcept
    {
        return ct::ordered_byte_map<Slot>::allocate_node(vtype, size);
    }
    void MemoryCache::SlotDeleter::operator()(Slot* slot) noexcept
    {
        ct::ordered_byte_map<Slot>::delete_node(slot);
    }

    bool MemoryCache::LockData::expired_auto() const noexcept
    {
        return std::chrono::system_clock::now() - timestamp > max;
    }
    bool MemoryCache::LockData::expired_man() const noexcept
    {
        return timestamp == std::chrono::system_clock::time_point();
    }
    bool MemoryCache::LockData::expired() const noexcept
    {
        return
            expired_man() ||
            expired_auto();
    }
    void MemoryCache::LockData::lock(Origin source) noexcept
    {
    ...
    void MemoryCache::LockData::unlock() noexcept
    {
        auto lock = ref(timestamp);
    ...
    std::size_t MemoryCache::LockData::allocation_size(std::chrono::system_clock::time_point, Origin) noexcept
    {
        return sizeof(LockData);
    }
    std::size_t MemoryCache::LockData::allocation_size() noexcept
    {
        return sizeof(LockData);
    }

    bool MemoryCache::Lock::is_ready() const noexcept
    {
        return _lock != nullptr;
    }
    void MemoryCache::Lock::wait() noexcept
    {
        if (_lock == nullptr)
            return;
        ...
        _schema(schema)
    {
        ...
        else
        {
            ...
            for (decltype(auto) it : std::filesystem::directory_iterator(_path/"flush"))
            {
                ...
                {
                    RDB_LOG(mem, "C", _id, " Detected corrupted flush",
            ...
            _disk_logs.replay([this](WriteType type, key_type key, View sort, View data)
            {
                ...
                {
                    RDB_TRACE(mem, "C", _id, " Replay - create partition <", uuid::encode(key, uuid::table_alnum), ">")
                    _create_partition_if(*_map, key, data);
                }
                else if (type == WriteType::Reset)
                {
                    RDB_TRACE(mem, "C", _id, " Replay - reset <", uuid::encode(key, uuid::table_alnum), ">")
                    _reset_impl(_find_partition(*_map, key), sort);
                    _flush_if();
                }
                else if (type == WriteType::Remov)
                {
                    RDB_TRACE(mem, "C", _id, " Replay - remove <", uuid::encode(key, uuid::table_alnum), ">")
                    _remove_impl(_find_partition(*_map, key), sort);
                    _flush_if();
                }
                else
                {
                    RDB_TRACE(mem, "C", _id, " Replay - write <", uuid::encode(key, uuid::table_alnum), ">")
    ...
    MemoryCache::FlushHandle& MemoryCache::_handle_open(std::size_t flush) const noexcept
    {
        ...
        {
            RDB_WARN("C", _id, " File resource limit")
                                 ...
                                 const read_callback& callback) noexcept
    {
        ...
        std::size_t found = 0;
        {
            ...
            {
                RDB_TRACE(mem, "C", _id, " Scanning RMPS")
            ...
            for (std::size_t j = _flush_id - flush_running; j > 0; j--)
            {
                ...
                if (handle.ready() && _bloom_may_contain(key, handle))
                {
                    ...
                    if (info.skeys())
                    {
                        ...
                        {
                            RDB_TRACE(mem, "C", _id, " Intra-partition bloom discard")
                            continue;
                        }
                        ...
                        if (sparse_block_offset.has_value())
                        {
                            ...
                            RDB_TRACE(mem, "C", _id, " Searching block sequence")
                            while (off < offset + partition_size)
                            {
                                        ...
                                        (result_max <= 0 && result_min >= 0))
                                {
                                    ...
                                    if (sparse_offset.has_value())
                                    {
                                        ...
                                        RDB_TRACE(mem, "C", _id, " Linear block search")
                                        ...
                                        do
                                        {
                                            ...
                                            {
                                                RDB_TRACE(mem, "C", _id, " Value removed")
                                                return false;
                                            }
                                            else
                                            {
                                            ...
                                            if (eq)
                                            {
                                                RDB_TRACE(mem, "C", _id, " Value found")
                                                ...
                                                {
                                                    RDB_TRACE(mem, "C", _id, " Continue search")
                                                    break;
                                                }
                                            }
                                            else
                                    ...
                                    {
                                        RDB_TRACE(mem, "C", _id, " Bloom miss")
                                    }
                                }
                                else
                                    off += compressed;
                        ...
                        {
                            RDB_TRACE(mem, "C", _id, " Intra-Bloom miss")
                        }
                    }
                    // Unary partition
                    else
    ...
    View MemoryCache::page_from(key_type key, const View& sort, std::size_t count) noexcept
    {
        ...
        if (info.skeys() && count > 0)
        {
            ...
            if (count)
            {
                ...
                for (std::size_t j = _flush_id - flush_running; j > 0; j--)
                {
                    RDB_TRACE(mem, "C", _id, "Searching F", j - 1)
            ...
            RDB_LOG(mem, "C", _id, "Size ", result.size(), "b <", uuid::encode(key, uuid::table_alnum), ">")
            return result;
        }
        return nullptr;
    }
            ...
            const View& pkey) noexcept
    {
        ...
        if (created)
        {
            RDB_LOG(mem, "C", _id, " Creating partition <", uuid::encode(key, uuid::table_alnum), ">")
    ...
    bool MemoryCache::unlock(key_type key, const View& sort, Origin origin) noexcept
    {
        ...
        if (lock->origin == origin)
        {
            ...
            lock->unlock();
            _lock_cnt--;
            RDB_WARN_IF(lock->expired(), mem, "C", _id, " Lock expired before unlock <", uuid::encode(key,
                        uuid::table_compact), ">")
            return res;
        }
        return false;

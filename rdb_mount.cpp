...
namespace rdb
{
    ...
    EventStore::ptr Mount::events() const noexcept
    {
    ...
    void Mount::_core_impl(std::size_t core)
    {
        ...
        for (decltype(auto) it : std::filesystem::directory_iterator(path))
        {
            ...
            schemas.emplace(
                ...
                std::forward_as_tuple(_shared, core, schema)
            );
        }
                    ...
                    t.queue.try_dequeue(task))
            {
                ...
                [[ unlikely ]] if (f == schemas.end())
                {
                    f = schemas.emplace(
                            ...
                            std::forward_as_tuple(_shared, core, task.first)
                        ).first;
                }
    ...
    void Mount::stop() noexcept
    {
        RDB_MODULE(mnt, "Attempting to stop");
        {
            ...
            for (decltype(auto) it : _threads)
            {
                ...
                if (it.thread.joinable())
                    it.thread.join();
                events()->trigger<Event::CoreStop>(i++);
            }
            _threads.clear();
            _status = Status::Stopped;
        }
    ...
    std::tuple<std::size_t, schema_type, const RuntimeSchemaReflection::RTSI*> Mount::_query_parse_op_rtsi(
    ...
    std::tuple<std::size_t, View, key_type> Mount::_query_parse_op_pkey(std::span<const unsigned char> packet,
    ...
    std::tuple<std::size_t, View> Mount::_query_parse_op_skey(std::span<const unsigned char> packet,
            const RuntimeSchemaReflection::RTSI& inf, ParserState& state, ParserInfo info) noexcept
    {
    ...
    std::size_t Mount::_query_parse_op_fetch(std::span<const unsigned char> packet, ParserState& state,
            ControlFlowInfo& cfi, ParserInfo info) noexcept
    {
        std::size_t off = 0;
    ...
    std::size_t Mount::_query_parse_op_create(std::span<const unsigned char> packet, ParserState& state,
            ControlFlowInfo& cfi, ParserInfo info) noexcept
    {
        std::size_t off = 0;
    ...
    std::size_t Mount::_query_parse_op_remove(std::span<const unsigned char> packet, ParserState& state,
            ControlFlowInfo& cfi, ParserInfo info) noexcept
    {
        std::size_t off = 0;
    ...
    std::size_t Mount::_query_parse_op_page(std::span<const unsigned char> packet, ParserState& state, ControlFlowInfo& cfi,
    ...
    std::size_t Mount::_query_parse_op_page_from(std::span<const unsigned char> packet, ParserState& state,
            ControlFlowInfo& cfi, ParserInfo info) noexcept
    {
        std::size_t off = 0;
    ...
    std::size_t Mount::_query_parse_op_check(std::span<const unsigned char> packet, ParserState& state,
            ControlFlowInfo& cfi, ParserInfo info) noexcept
    {
        std::size_t off = 0;
    ...
    std::size_t Mount::_query_parse_op_if(std::span<const unsigned char> packet, ParserState& state, ControlFlowInfo&,
                                          ParserInfo info) noexcept
    {
        std::size_t off = 0;
    ...
    std::size_t Mount::_query_parse_op_atomic(std::span<const unsigned char> packet, ParserState& state,
            ControlFlowInfo& cfi, ParserInfo info) noexcept
    {
        std::size_t off = 0;
    ...
    std::size_t Mount::_query_parse_op_lock(std::span<const unsigned char> packet, ParserState& state, ControlFlowInfo&,
                                            ParserInfo info) noexcept
    {
        std::size_t off = 0;
    ...
    std::size_t Mount::_query_parse_op_barrier(std::span<const unsigned char> packet, ParserState& state,
            ControlFlowInfo& cfi, ParserInfo info) noexcept
    {
        state.wait();
        return 0;
    ...
    std::size_t Mount::_query_parse_operand(std::span<const unsigned char> packet, ParserState& state,
                                            ParserInfo info) noexcept
    {
        std::size_t off = 0;
        [[ likely ]] if (const auto op = packet[off++]; op < _op_parse_table.size())
    ...
    std::size_t Mount::_query_parse_schema_operator(std::span<const unsigned char> packet, key_type key, View partition,
    ...
    std::size_t Mount::_query_parse_predicate_operator(std::span<const unsigned char> packet, key_type key, View partition,
            View sort, schema_type schema, ParserState& state, ControlFlowInfo& cfi, ParserInfo& info) noexcept
    {
        auto& core = _threads[_vcpu(key)];
        std::size_t off = 0;
